...
type BeforeToolCallback func(ctx tool.ToolContext, tool tool.Tool, args map[string]any) (map[string]any, error)

type AfterToolCallback func(ctx tool.ToolContext, tool tool.Tool, args, result map[string]any, err error) (map[string]any, error)

type OnToolErrorCallback func(ctx tool.ToolContext, tool tool.Tool, args map[string]any, err error) (map[string]any, error)
...
func toolPreprocess(ctx agent.InvocationContext, req *model.LLMRequest, tools []tool.Tool) error {
	for _, t := range tools {
		...
		toolCtx := tool.NewToolContext(ctx, "", &session.EventActions{}, nil)
...
func toolsetPreprocess(ctx agent.InvocationContext, req *model.LLMRequest) error {
	...
	for _, toolset := range Reveal(llmAgent).Toolsets {
		...
		toolCtx := tool.NewToolContext(ctx, "", nil, nil)
...
func (f *Flow) handleFunctionCalls(ctx agent.InvocationContext, toolsDict map[string]tool.Tool, resp *model.LLMResponse, toolConfirmations map[string]*toolconfirmation.ToolConfirmation, liveSess agent.LiveSession) (mergedEvent *session.Event, err error) {
	...
	for i, fnCall := range fnCalls {
		...
		go func(i int, fnCall *genai.FunctionCall) {
			...
			toolCtx := tool.NewToolContext(toolCallCtx, fnCall.ID, &session.EventActions{StateDelta: make(map[string]any)}, confirmation)
...
func (f *Flow) runOnToolErrorCallbacks(toolCtx tool.ToolContext, tool tool.Tool, fArgs map[string]any, err error) (map[string]any, error) {
...
func (f *Flow) callTool(toolCtx tool.ToolContext, tool toolinternal.FunctionTool, fArgs map[string]any) map[string]any {
...
func (f *Flow) invokeBeforeToolCallbacks(toolCtx tool.ToolContext, tool tool.Tool, fArgs map[string]any) (map[string]any, error) {
...
func (f *Flow) invokeAfterToolCallbacks(toolCtx tool.ToolContext, tool toolinternal.FunctionTool, fArgs, fResult map[string]any, fErr error) (map[string]any, error) {
...
func (f *Flow) invokeOnToolErrorCallbacks(toolCtx tool.ToolContext, tool tool.Tool, fArgs map[string]any, fErr error) (map[string]any, error) {
...
type pluginManager interface {
	...
	RunBeforeToolCallback(ctx tool.ToolContext, t tool.Tool, args map[string]any) (map[string]any, error)
	RunAfterToolCallback(ctx tool.ToolContext, t tool.Tool, args, result map[string]any, err error) (map[string]any, error)
	RunOnToolErrorCallback(ctx tool.ToolContext, t tool.Tool, args map[string]any, err error) (map[string]any, error)
}
