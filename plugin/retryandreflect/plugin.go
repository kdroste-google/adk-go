...
func (r *retryAndReflect) afterTool(ctx tool.ToolContext, tool tool.Tool, args, result map[string]any, err error) (map[string]any, error) {
	if err == nil {
...
func (r *retryAndReflect) onToolError(ctx tool.ToolContext, tool tool.Tool, args map[string]any, err error) (map[string]any, error) {
	return r.handleToolError(ctx, tool, args, err)
}

func (r *retryAndReflect) handleToolError(ctx tool.ToolContext, failedTool tool.Tool, args map[string]any, err error) (map[string]any, error) {
...
func (r *retryAndReflect) scopeKey(ctx tool.ToolContext) string {
	if r.scope == Global {
...
func (r *retryAndReflect) resetFailuresForTool(ctx tool.ToolContext, toolName string) {
	scopeKey := r.scopeKey(ctx)
