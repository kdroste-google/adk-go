...
// Command server runs an A2A (Agent-To-Agent) server that acts as an approval
// authority: it decides, on the user's behalf, whether a proposed action should
// be allowed.
//
// This is the first half of a paired example. The second half lives in
// ../client. There the confirmation-requiring tool runs *locally on the
// client*; when it needs confirmation the client does not ask a human, it asks
// this server. The server answers APPROVE/DENY, effectively impersonating the
// user and supplying the confirmation decision remotely.
//
// Run it with:
//
//	GOOGLE_API_KEY=... go run ./examples/a2aconfirmation/server
//
// then, in another terminal, run the client (see ../client).
package main
...
func main() {
	addr := flag.String("addr", "127.0.0.1:8080", "host:port to serve the A2A server on")
	flag.Parse()
	...
	log.Printf("[server] A2A approval agent listening on %s", baseURL.String())
	log.Printf("[server] agent card: %s", baseURL.JoinPath(a2asrv.WellKnownAgentCardPath).String())
	log.Printf("[server] point the client at: %s", baseURL.String())

	if err := http.Serve(listener, mux); err != nil {
		log.Fatalf("A2A server stopped: %v", err)
	}
}
